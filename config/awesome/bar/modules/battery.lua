...
function battery_widget:discover()
    ...
    for adapter in io.popen("ls -1 " .. pow):lines() do
        local type = utils.read_trim(pow .. adapter .. "/type")
...
function battery_widget:get_state()
    ...
    -- return value
    local r = {
        state     = tolower (utils.read_trim(bat.."/"..sysfs.state)),
        present   = tonumber(utils.read_trim(bat.."/"..sysfs.present)),
        rate      = tonumber(utils.read_trim(bat.."/"..sysfs.rate)),
        charge    = tonumber(utils.read_trim(bat.."/"..sysfs.charge)),
        capacity  = tonumber(utils.read_trim(bat.."/"..sysfs.capacity)),
        design    = tonumber(utils.read_trim(bat.."/"..sysfs.design)),
        percent   = tonumber(utils.read_trim(bat.."/"..sysfs.percent)),
    }

    r.ac_state = tonumber(utils.read_trim(ac.."/online"))
...
function battery_widget:update()
    ...
    ctx.AC_BAT  = (ctx.ac_state == 1
                   and utils.lookup_by_limits(self.ac_prefix, ctx.percent)
                   or utils.lookup_by_limits(self.battery_prefix, ctx.percent)
                   or "Err!")

    -- Colors
    ctx.color_on, ctx.color_off = utils.color_tags(
        utils.lookup_by_limits(self.percent_colors, ctx.percent))
    ...
    if ctx.capacity and ctx.design then
        ctx.capacity_percent = utils.round(ctx.capacity/ctx.design*100)
    end
    ...
    -- update text
    self.widget:set_markup(utils.substitute(self.widget_text, ctx))
    self.tooltip:set_text(utils.substitute(self.tooltip_text, ctx))

    -- low battery notification
    if naughty then
        if (ctx.state == "discharging" and
                ctx.percent and ctx.percent <= self.alert_threshold) then
            self:notify(utils.substitute(self.alert_title, ctx),
                        utils.substitute(self.alert_text, ctx))
