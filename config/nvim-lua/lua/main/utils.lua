...
function utils.load_config(reload)
  if base_config ~= nil and not reload then
    return base_config
  else
...
function utils.hide_statusline() 
   ...
   local statusline = utils.load_config().ui.statusline

   if vim.tbl_contains(statusline.shown, buftype) then
      api.nvim_set_option("laststatus", 2)
      return
   end

   if vim.tbl_contains(statusline.hidden, buftype) then
...
function utils.close_buffer(bufexpr, force)
